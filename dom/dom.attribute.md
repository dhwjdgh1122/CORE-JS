...
그런데 아래 코드에선 `input.value` 처럼 동기화가 속성에서 프로퍼티 방향으로만 일어나는 예외상황도 존재한다.
...
몇 가지 다른 예를 살펴보자 `style` 속성의 경우 문자열이지만, `style` 프로퍼티는 객체이다.
...
대부분의 프로퍼티의 값은 문자열이다. 아주 드물긴 하지만, DOM 프로퍼티 값이 문자열이더라도 속성값과 다른 경우도 있다 `href` 속성이 상대 URL이나 `#hash`이더라도 `href` DOM 프로퍼티엔 항상 URL 전체가 저장되는 경우가 대표적인 예이다.
...
HTML을 작성할 때 우리는 대부분 표준 속성을 사용한다. 하지만 표준이 아닌 속성도 존재한다. 이런 비표준이 유용한 지 아닌 지, 그리고 어떤 경우에 비표준 속성을 사용해야 하는지 알아보자
...
아래 코드에선 주문 상태(order state)를 나타내는 커스텀 속성 `order-state`를 사용해 주문 상태에 따라 스타일을 변경한다.
...
이렇게 커스텀 속성을 사용하는 게 `.order-state-new`, `.order-state-pending`, `order-state-canceled`같은 클래스를 사용하는 것보다 왜 선호될까?
...
**data**로 시작하는 속성 전체는 개발자가 용도에 맞게 사용하도록 별도록 예약된다. `dataset` 프로퍼티를 사용하면 이 속성에 접근 할 수 있다.
...
이런 특징을 사용해 주문 상태에 관련 코드를 다시 작성하면 아래와 같다.
...
`data-*` 속성을 사용하면 읽기 뿐만 아니라 수정도 가능하다는 점에 유의하자. 속성이 수정되면 CSS가 해당 뷰를 자동으로 업데이트 해준다. 위 예시에서 `(*)` 로 표시한 줄에서 색이 파란색으로 바뀐다.
...
<img src="Dom6.png" width="650" height="100" >
